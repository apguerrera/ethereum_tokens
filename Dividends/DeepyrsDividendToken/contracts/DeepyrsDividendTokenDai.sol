...
contract DividendToken is IERC20, Owned {
    ...
    uint256 constant POINTS_PER_DAI = 1e32;
    uint256 constant DAI_ETH = 100; // replace with MakerDao pricefeed
    ...
    constructor() public {
      ...
      daiAddress = IERC20(0x89d24A6b4CcB1B6fAA2625fE562bDD9a23260359);
    }
    ...
    */
    ...
    function setDaiAddress (address _token) public onlyOwner {
        require(_token != address(0));
        daiAddress = IERC20(_token);
    ...
    function dividendsOwing(address account) public view returns(uint256) {   // public for testing, was originally private
      ...
      return (accounts[account].balance * newDividendPoints) / POINTS_PER_DAI;
    }
        ...
        returns (uint _amount)
    {
        ...
        _amount = creditedPoints[msg.sender] / POINTS_PER_DAI;
        creditedPoints[msg.sender] = 0;
        dividendsCollected += _amount;
        emit CollectedDividends(now, msg.sender, _amount);
        require(msg.sender.call.value(_amount)()); // replace with DAI
    }
    ...
    function getOwedDividends(address _account) public constant returns (uint _amount) {
        return (_getUncreditedPoints(_account) + creditedPoints[_account])/POINTS_PER_DAI;
    }

    function sweepDust() public onlyOwner {
        if (ethDust > 0) {
           uint ethAmount = ethDust;
           ethDust = 0;
           require(transfer(msg.sender, ethAmount));
        }
        if (daiDust > 0) {
           uint daiAmount = daiDust;
           daiDust = 0;
           require(transfer(msg.sender, daiAmount));
        }
    }
    ...
    function depositDAI(uint256 _amount) public {
      if (_amount == 0) return;
      uint256 dust = _amount % totalSupply();
      uint256 amount = _amount - dust;
      daiDust += dust;

      IERC20 token = IERC20(daiAddress); // Change to DAI
      require(IERC20(token).transfer(this, amount));
      _depositDividends(_amount);
    }

    function _depositDividends(uint256 amount) internal {
      // POINTS_PER_DAI is 1e32.
      // So, no multiplication overflow unless msg.value > 1e45 wei (1e27 DAI)
      totalDividendPoints += (amount * POINTS_PER_DAI ) / totalSupply();
    ...
    function () payable public {
        ...
        // replace with Eth to Dai converter, and pass the dai to depositDai()
        _depositDividends(amount * DAI_ETH);
    }
